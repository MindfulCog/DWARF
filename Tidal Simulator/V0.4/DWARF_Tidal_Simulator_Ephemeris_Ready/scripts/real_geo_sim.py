...
class RegionalGeoSim(DWARFTidalSimulator):
    ...
    def run_step(self, step):
        ...
        for i in range(self.num_tracers):
            ...
            moon_force = self.moon_mass / (r_moon_mag**2 + 1e6) * (r_moon / (r_moon_mag + 1e-6))

            # Sun force
            r_sun = sun_pos - tracer
            r_sun_mag = np.linalg.norm(r_sun)
            sun_force = self.sun_mass / (r_sun_mag**2 + 1e10) * (r_sun / (r_sun_mag + 1e-6))
...
def run_real_geo_sim():
    ...
    sim = RegionalGeoSim(region_bounds=region, grid_res=50, mask_path=mask_path)
