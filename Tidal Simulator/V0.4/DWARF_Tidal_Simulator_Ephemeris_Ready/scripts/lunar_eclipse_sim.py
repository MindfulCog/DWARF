...
class LunarEclipseSim(DWARFTidalSimulator):
    ...
    def run_step(self, step):
        ...
        sun_angle = moon_angle + np.pi  # Earth is between Sun and Moon
        ...
        for i in range(self.num_tracers):
            ...
            # Sun-induced flow (partially blocked by Earth)
            r_sun = sun_pos - tracer
            r_sun_mag = np.linalg.norm(r_sun)
            shadow_factor = 1.0 if np.linalg.norm(tracer - earth_pos) > self.earth_radius else 0.5
            sun_pressure = shadow_factor * self.sun_mass / (r_sun_mag**2 + 1e10)
...
def run_lunar_eclipse_sim():
    print("🌕 Running Lunar Eclipse DWARF Simulation...")
    sim = LunarEclipseSim(num_tracers=500, total_steps=200)
    sim.run()
    plot_displacement(sim, filename="lunar_eclipse_displacement.png")
    plot_snapshot(sim, filename="lunar_eclipse_snapshot.png")
    export_all_3d(sim, output_dir="output/3d/lunar_eclipse")
    log_dynamics(sim, filename="lunar_eclipse_dynamics.csv")
    print("✅ Lunar Eclipse Simulation Complete.")

if __name__ == "__main__":
    run_lunar_eclipse_sim()
