...
# Import core DWARF modules
from physics_core import DwarfVortexSimulator
...
def update_visualization(fig, axes, vortex_simulator, quantum_tracker, 
                         trajectory, energy_history, time_history):
    """
    ...
    vortex_ax.scatter([0], [0], [0], c='b', s=100)
    ...
    if len(trajectory) > 100:
        ...
        if grid is not None and density is not None:
            ...
            if points:
                ...
                # Plot probability clouds as points
                prob_ax.scatter(points[:, 0], points[:, 1], points[:, 2], 
                              c=densities, cmap='viridis', alpha=0.3, s=10*densities)
    
    # Plot proton at origin
    prob_ax.scatter([0], [0], [0], c='r', s=100)
    ...
    if state:
        ...
        if coupling_info["detected"]:
            info_text += (
                ...
                f"===========================\n"
                f"Spin-orbit angle: {coupling_info['angle']:.1f}°\n"
