    ...
    def _simulation_loop(self):
        ...
        try:
            while not self.stop_event.is_set():
                ...
                # Record physics calculation time
                self.perf_metrics['physics_time'] = time.time() - physics_start
                
                # Update simulation FPS counter
                self.sim_frame_count += 1
                now = time.time()
                if now - self.last_metrics_update >= 1.0:
                    self.perf_metrics['sim_fps'] = self.sim_frame_count / (now - self.last_metrics_update)
                    self.sim_frame_count = 0
                    self.last_metrics_update = now
                    print(f"Simulation FPS: {self.perf_metrics['sim_fps']:.1f}, " +
                          f"Physics: {self.perf_metrics['physics_time']*1000:.1f}ms, " + 
                          f"Transfer: {self.perf_metrics['transfer_time']*1000:.1f}ms")
