...
try:
    ...
    print("Warning: Some constants not found in config.py, trying to import from SimulationConfig")
    try:
        ...
        print("Error: Could not load simulation constants! Using defaults.")
...
def apply_spin_effects(particles):
    ...
    for p in particles:
        ...
        # Calculate angular momentum (L = r x p) - physical quantity
        try:
            center = np.array([GRID_SIZE/2, GRID_SIZE/2])
